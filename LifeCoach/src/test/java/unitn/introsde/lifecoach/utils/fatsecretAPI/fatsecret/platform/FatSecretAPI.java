...
public class FatSecretAPI {
	...
	public String getFoodById(String food_id) throws FatSecretException, IOException{
		String urlBase = URL_BASE + "method=food.get&food_id="+food_id+"&format=json";
		System.out.println(urlBase);
		OAuthBase oAuth = new OAuthBase();
		URL url = new URL(urlBase);
		Result result = new Result();
		
		oAuth.generateSignatureForGet(url, _consumerKey, _consumerSecret, null, null, result);
		String response = doGETHttpMethodReq(result.getNormalizedUrl(), "GET", result.getNormalizedRequestParameters() + "&" + OAuthBase.OAUTH_SIGNATURE + "=" + URLEncoder.encode(result.getSignature(), "utf-8"), null);
		System.out.println(result.getNormalizedUrl());
		System.out.println(result.getNormalizedUrl()+"&"+result.getNormalizedRequestParameters() + "&" + OAuthBase.OAUTH_SIGNATURE + "=" + URLEncoder.encode(result.getSignature(), "utf-8"));
		System.out.println(URL_BASE+result.getNormalizedRequestParameters() + "&" + OAuthBase.OAUTH_SIGNATURE + "=" + URLEncoder.encode(result.getSignature(), "utf-8"));
		return response;
	}
    ...
    private String doGETHttpMethodReq(String urlStr, String requestMethod, String paramStr, Map<String, String> header) throws FatSecretException, IOException{
    	String url = URL_BASE+paramStr;
	...
	private static class OAuthBase{
		...
		public void generateSignatureForGet(URL url, String consumerKey, String consumerSecret, String token, String tokenSecret, Result result) {
